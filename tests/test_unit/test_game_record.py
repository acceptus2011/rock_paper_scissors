...
class TestAddRecord(TestCase):
    ...
    def test_equal_name_and_more_scores(
            self) -> None:  # Проверка на запись игрока с одинаковым модом но большим количеством очков
    ...
    def test_equal_name_and_less_scores(
            self) -> None:  # Проверка на запись игрока с одинаковым модом но меньшим количеством очков
        self.game.add_record(PlayerRecord("test_name1", MODE_HARD, 8))
...
class TestPrepareRecords(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.test_records = [player1, player2, player3, player4,
                             player5]  # отсортированный список записей длинной MAX_RECORDS_NUMBER
        self.game = GameRecord()
        self.game.records = []
        self.game.add_record(PlayerRecord("test_name1", MODE_NORMAL, 6))
        self.game.add_record(PlayerRecord("test_name", MODE_HARD, 10))
